...
def main(file = "interval.txt"):
    # Each list begins with a new line
    
    fp = open(file)
    line = fp.readline()
    A = line.split(" ") 
    L1 = [ [int(A[i]), int(A[i+1])] for i in range(0, len(A), 2) ]
    line = fp.readline()
    A = line.split(" ") 
    L2 = [ [int(A[i]), int(A[i+1])] for i in range(0, len(A), 2) ]    
    
    Tp = compareList(L1,L2)
    
    print Tp

if __name__=="__main__":
    main(sys.argv[1] if len(sys.argv) > 1 else "interval.txt")
